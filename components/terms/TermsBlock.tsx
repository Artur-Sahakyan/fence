        ...
        <div className="max-w-3xl flex flex-col gap-4">
          {data.map((item, idx) => (
            <div key={idx} className="flex flex-col gap-4">
              ...
              {item.desc && (
                <p className="text-black text-sm leading-[21px]">{item.desc}</p>
              )}
              {item.list && (
                <ul
                  className={`text-primary-grey text-sm pl-8 ${
                    item.numberList ? "list-decimal" : "list-disc"
                  }`}
                >
                  {item.list.map((listItem, listIdx) => (
                    <li key={listIdx} className="mb-3 last:mb-0">
                      {listItem}
                    </li>
                  ))}
                </ul>
              ...
              {item.short_desc && (
                <p className="text-primary-grey text-sm leading-[21px]">{item.short_desc}</p>
              )}
            </div>
          ))}
